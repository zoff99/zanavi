...
static void
graphics_draw_polyline_as_polygon(struct graphics *gra, struct graphics_gc *gc, struct point *pnt, int count, int *width, int step, int fill, int order, int oneway)
{
	int maxpoints=200;
	...
	for (;;) {
		...
		case 1:
		       if (fowo != fow) {
				...
				if (delta < 512) {
					...
					res[--npos] = nego;
					--npos;
					if (fill==1) draw_circle(&pnt[i], wi, prec, fowo-512, -delta, res, &npos, -1);
					res[npos] = neg;
				} else {
					res[ppos++] = poso;
					if (fill==1) draw_circle(&pnt[i], wi, prec, fowo, 1024-delta, res, &ppos, 1);
					res[ppos++] = pos;
		...
		case 3:
			res[--npos] = neg;
			--npos;
			if (fill==1) draw_circle(&pnt[i], wi, prec, fow-512, -512, res, &npos, -1);
			res[npos] = pos;
			res[ppos++] = pos;
			dbg_assert(npos > 0);
		...
		case 0:
			res[ppos++] = neg;
			if (fill==1) draw_circle(&pnt[i], wi, prec, fow+512, 512, res, &ppos, 1);
			res[ppos++] = pos;
			break;
		}
		i++;
...
static void
graphics_draw_polyline_clipped(struct graphics *gra, struct graphics_gc *gc, struct point *pa, int count, int *width, int step, int poly, int order, int oneway)
{
		...
		if (i) {
			...
			if (i == count-1 || (code & 4)) {
				if (out > 1) {
					...
					{
						gra->meth.draw_lines2(gra->priv, gc->priv, p, out, order, 0);
					}
					out=0;
				}
			}
...
static void
displayitem_draw(struct displayitem *di, void *dummy, struct display_context *dc, int order, int allow_dashed)
{
	//dbg(0,"ooo enter ooo\n");
...
static void xdisplay_draw(struct displaylist *display_list, struct graphics *gra, struct layout *l, int order)
{
	...
	//dbg(0,"draw prerender map for drag!!\n");
	...
	if (!draw_vector_map)
	{
		...
		//dbg(0,"###### screen center x=%d\n",t->screen_center.x);
		//dbg(0,"###### screen center y=%d\n",t->screen_center.y);
		...
		struct coord *c;
		c=&(t->map_center);
		//dbg(0,"###### map center x=%d\n",c->x);
		//dbg(0,"###### map center y=%d\n",c->y);
		...
		//dbg(0,"xxx 5 44444xxxx=%d\n",mcenter_pnt.x);
		//dbg(0,"yyy 5 44444yyyy=%d\n",mcenter_pnt.y);
		...
		dbg(0,"left top lat=%f, lng=%f, x=%d y=%d\n",g99.lat,g99.lng,c99.x,c99.y);
		//dbg(0,"left top lat=%f, lng=%f, x=%f y=%f\n",g99.lat,g99.lng,c99.x,c99.y);
		//dbg(0,"0x%x,0x%x\n",c99.x,c99.y);
		...
		g99.lng=170.0;
		transform_from_geo(pro, &g99, &c99);
		dbg(0,"right bottom lat=%f, lng=%f, x=%d y=%d\n",g99.lat,g99.lng,c99.x,c99.y);
		//dbg(0,"right bottom lat=%f, lng=%f, x=%f y=%f\n",g99.lat,g99.lng,c99.x,c99.y);
		transform(global_navit->trans, pro, &c99, &cursor_pnt, 1, 0, 0, NULL);
		dbg(0,"xxxxxxx=%d\n",cursor_pnt.x);
		dbg(0,"yyyyyyy=%d\n",cursor_pnt.y);
		*/
		...
		g99.lng=0.0;
		transform_from_geo(pro, &g99, &c99);
		//dbg(0,"center center lat=%f, lng=%f, x=%d y=%d\n",g99.lat,g99.lng,c99.x,c99.y);
		//dbg(0,"center center lat=%f, lng=%f, x=%f y=%f\n",g99.lat,g99.lng,c99.x,c99.y);
		transform(global_navit->trans, pro, &c99, &cursor_pnt, 1, 0, 0, NULL);
		//dbg(0,"xxx44444xxxx=%d\n",cursor_pnt.x);
		//dbg(0,"yyy44444yyyy=%d\n",cursor_pnt.y);
...
do_draw(struct displaylist *displaylist, int cancel, int flags)
{
	...
	if (displaylist->order != displaylist->order_hashed || displaylist->layout != displaylist->layout_hashed)
	{
		...
		// dbg(0,"-----====> order (zoom) = %d",displaylist->order);
		// dbg(0,"-----====> order (zoom) = %d",displaylist->order_hashed);
	...
	if (!draw_vector_map)
	{
		...
		//dbg(0,"2 lat=%f, lng=%f\n",g22.lat,g22.lng);
		transform(global_navit->trans, pro, c22, &mcenter_pnt, 1, 0, 0, NULL);
		//dbg(0,"xxx 5 33333xxxx=%d\n",mcenter_pnt.x);
		//dbg(0,"yyy 5 33333yyyy=%d\n",mcenter_pnt.y);
	...
	if (! cancel)
	{
		...
		// dbg(0,"call graphics_displaylist_draw 3")
