...
static int
ascii_cmp_local(char *name, char *match, int partial)
{
...
void
search_full_world(char *addr, int partial, int search_order, struct jni_object *jni,struct coord_geo *search_center, int search_radius)
{
	struct item *item;
					...
					if (mr)
					{
						...
						while ((item=map_rect_get_item(mr)))
						{
								...
								else if (item_is_street(*item))
								{
									...
									else if (item_attr_get(item, attr_street_name_systematic, &attr))
									{
...
//	search_full_world(addr,partial,jni,search_country_flags,search_country_string);
	...
	while (tmp)
	{
		...
		attr.u.str=tmp->data;
		search_list_search(sl, &attr, partial);
		result_list=search_address__country(result_list, sl, phrases, tmp, partial, jni);
		tmp=g_list_next(tmp);
	}
