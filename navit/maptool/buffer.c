...
void free_buffer(char *filename, struct buffer *b)
{
	if (b->base)
	{
		if (verbose_mode) fprintf(stderr, "free_buffer:# freeing(3) buffer #\n");

		if (verbose_mode) fprintf(stderr,"free_buffer:info has b->malloced=%d\n", b->malloced);
		if (verbose_mode) fprintf(stderr,"free_buffer:info     has b->size="LONGLONG_FMT"\n", (size_t)b->size);
		if (verbose_mode) fprintf(stderr,"free_buffer:info            base="LONGLONG_FMT"\n", b->base);
...
void load_buffer_fp(FILE *f, struct buffer *b, long long offset, long long size)
{
	...
	if (b->base)
	{
		...
		if (size > b->size)
		{
			if (verbose_mode) fprintf(stderr, "load_buffer:# freeing(1) buffer #\n");
			free(b->base);
...
#if 1
		// now always free and malloc a fresh buffer
		if (verbose_mode) fprintf(stderr, "load_buffer:# freeing(2.a) buffer #\n");
		free(b->base);
		if (verbose_mode) fprintf(stderr, "load_buffer:# freeing(2.b) buffer #\n");
		b->base = NULL;
#endif
