...
char ib_buffer_array[MAX_THREADS][800000];
...
void sql_create_index001()
{
	...
	sqlite3_exec(sql_handle004, "PRAGMA cache_size = -30000", NULL, NULL, &errorMessage);
	sqlite3_exec(sql_handle005, "PRAGMA temp_store=MEMORY", NULL, NULL, &errorMessage);
	sqlite3_exec(sql_handle005, "PRAGMA cache_size = -30000", NULL, NULL, &errorMessage);
	sqlite3_exec(sql_handle006, "PRAGMA temp_store=MEMORY", NULL, NULL, &errorMessage);
	sqlite3_exec(sql_handle006, "PRAGMA cache_size = -30000", NULL, NULL, &errorMessage);
	sqlite3_exec(sql_handle007, "PRAGMA temp_store=MEMORY", NULL, NULL, &errorMessage);
	sqlite3_exec(sql_handle007, "PRAGMA cache_size = -30000", NULL, NULL, &errorMessage);
	...
	sqlite3_exec(sql_handle, "PRAGMA cache_size = -30000", NULL, NULL, &errorMessage);
...
void sql_db_init(int startup)
{
	...
	if (startup == 1)
	{
		...
		char create_table1[200] = "CREATE TABLE IF NOT EXISTS way (id integer, town_id integer, ind integer, lat real,lon real, name text, name_fold text)";
		retval = sqlite3_exec(sql_handle, create_table1, 0, 0, 0);

		char create_table2[200] = "CREATE TABLE IF NOT EXISTS way_node(way_id integer,node_id integer, s integer)";
	...
	sqlite3_exec(sql_handle, "PRAGMA cache_size = -40000", NULL, NULL, &errorMessage);
	// WAYS and TOWNS -------
	...
	sqlite3_exec(sql_handle002a, "PRAGMA cache_size = -80000", NULL, NULL, &errorMessage);
	...
	sqlite3_exec(sql_handle002b, "PRAGMA cache_size = -80000", NULL, NULL, &errorMessage);
	// NODE -------
	...
	sqlite3_exec(sql_handle003a, "PRAGMA cache_size = -80000", NULL, NULL, &errorMessage);
	...
	sqlite3_exec(sql_handle003b, "PRAGMA cache_size = -80000", NULL, NULL, &errorMessage);
	// NODE -------
	...
	sqlite3_exec(sql_handle004, "PRAGMA cache_size = -100000", NULL, NULL, &errorMessage);
	// WAY NODE -------
	...
	sqlite3_exec(sql_handle005, "PRAGMA cache_size = -100000", NULL, NULL, &errorMessage);
	// WAY NODE -------
	...
	sqlite3_exec(sql_handle006, "PRAGMA cache_size = -100000", NULL, NULL, &errorMessage);
	// WAY NODE -------
	...
	sqlite3_exec(sql_handle007, "PRAGMA cache_size = -100000", NULL, NULL, &errorMessage);
	// WAY NODE -------
	...
	//fprintf(stderr, "prep:%d\n", retval);
	retval = sqlite3_prepare_v2(sql_handle, "INSERT INTO way (id, name, town_id, lat, lon, name_fold, ind) VALUES (?,?,?,?,?,?,0);", -1, &stmt_way, NULL);
	fprintf(stderr, "prep:%d\n", retval);
	retval = sqlite3_prepare_v2(sql_handle004, "INSERT INTO way_node (way_id, node_id,s) VALUES (?,?,?);", -1, &stmt_way_node, NULL);
	fprintf(stderr, "prep:%d\n", retval);
	retval = sqlite3_prepare_v2(sql_handle005, "INSERT INTO way_node (way_id, node_id,s) VALUES (?,?,?);", -1, &stmt_way_node__2, NULL);
	fprintf(stderr, "prep:%d\n", retval);
	retval = sqlite3_prepare_v2(sql_handle006, "INSERT INTO way_node (way_id, node_id,s) VALUES (?,?,?);", -1, &stmt_way_nodeb, NULL);
	fprintf(stderr, "prep:%d\n", retval);
	retval = sqlite3_prepare_v2(sql_handle007, "INSERT INTO way_node (way_id, node_id,s) VALUES (?,?,?);", -1, &stmt_way_node__2b, NULL);
	fprintf(stderr, "prep:%d\n", retval);
	...
	retval = sqlite3_prepare_v2(sql_handle, "select t.id,w.lat, w.lon, w.name, t.country_id from way w left outer join town t on w.town_id=t.id where w.ind = 0 and w.name_fold like ? order by w.name_fold,w.town_id;", -1, &stmt_sel003, NULL);
	fprintf(stderr, "prep:%d\n", retval);

	retval = sqlite3_prepare_v2(sql_handle, "update way set ind = ? where ind = 0 and name_fold like ?;", -1, &stmt_sel003u, NULL);
	fprintf(stderr, "prep:%d\n", retval);
...
where ind = 0 \
and w.name_fold not like 'a%' \
and w.name_fold not like 'b%' \
and w.name_fold not like 'c%' \
and w.name_fold not like 'd%' \
and w.name_fold not like 'e%' \
and w.name_fold not like 'f%' \
and w.name_fold not like 'g%' \
and w.name_fold not like 'h%' \
and w.name_fold not like 'i%' \
and w.name_fold not like 'j%' \
and w.name_fold not like 'k%' \
and w.name_fold not like 'l%' \
and w.name_fold not like 'm%' \
and w.name_fold not like 'n%' \
and w.name_fold not like 'o%' \
and w.name_fold not like 'p%' \
and w.name_fold not like 'q%' \
and w.name_fold not like 'r%' \
and w.name_fold not like 's%' \
and w.name_fold not like 't%' \
and w.name_fold not like 'u%' \
and w.name_fold not like 'v%' \
and w.name_fold not like 'w%' \
and w.name_fold not like 'x%' \
and w.name_fold not like 'y%' \
and w.name_fold not like 'z%' \
...
int main(int argc, char **argv)
{
	...
	while (1)
	{
		...
		static struct option long_options[] = { { "md5", 1, 0, '5' }, { "64bit", 0, 0, '6' }, { "attr-debug-level", 1, 0, 'a' }, { "binfile", 0, 0, 'b' }, { "compression-level", 1, 0, 'z' },
				...
				{	"db", 1, 0, 'd'},
#endif
				{ "dedupe-ways", 0, 0, 'w' }, { "dump", 0, 0, 'D' }, { "dump-coordinates", 0, 0, 'c' }, { "end", 1, 0, 'e' }, { "help", 0, 0, 'h' }, { "keep-tmpfiles", 0, 0, 'k' }, { "nodes-only", 0, 0, 'N' }, { "map", 1, 0, 'm' }, { "plugin", 1, 0, 'p' },
				// { "protobuf", 0, 0, 'P' },
		...
		c = getopt_long(argc, argv, "5:6B:DF:NO:PWS:a:bc"
				...
				"d:"
#endif
					"e:fghj:i:knm:p:r:s:wvu:z:UXYZ", long_options, &option_index);
		if (c == -1)
		{
			break;
		}
	...
	fprintf(stderr, "PROGRESS: Phase 9: take attr housenumber and add as housenumber node\n");
	...
	fprintf(stderr, "-RUNTIME-PHASE9: %s\n", outstring3);
	// ------ take attr housenumber from ways (building=yes) and add as housenumber node -----
