...
void relations_process(struct relations *rel, FILE *nodes, FILE *ways, FILE *relations)
{
	...
	if (nodes)
	{
		...
		long long pos_now = ftello(nodes); // 64bit
		fseeko(nodes, 0, SEEK_END);
		size_in = ftello(nodes); // 64bit
		fseeko(nodes, pos_now, SEEK_SET);
		// reset timer
		diff2_tt = 0;
		_c = 0;
		time(&start_tt);
	...
	if (ways)
	{
		...
		long long pos_now = ftello(ways); // 64bit
		fseeko(ways, 0, SEEK_END);
		size_in = ftello(ways); // 64bit
		fseeko(ways, pos_now, SEEK_SET);
		// reset timer
		diff2_tt = 0;
		_c = 0;
		time(&start_tt);
