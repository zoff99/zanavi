...
binfile_read_cd(struct map_priv *m, int offset, int len)
{
	...
	if (cd)
	{
		//dbg(0,"cd at "LONGLONG_FMT" %d bytes\n",cdoffset+offset, sizeof(*cd)+len);
		cd_to_cpu(cd);
		//dbg(0, "sig 0x%x\n", cd->zipcensig);
...
static int binfile_search_cd(struct map_priv *m, int offset, char *name, int partial, int skip)
{
	...
	while (offset < end)
	{
		...
		if (!m->search_data || m->search_offset > offset || offset - m->search_offset + sizeof(*cd) > m->search_size || offset - m->search_offset + sizeof(*cd) + cd->zipcfnl + cd->zipcxtl > m->search_size)
		{
#if 0
...
{ binfile_coord_rewind, binfile_coord_get, binfile_attr_rewind, binfile_attr_get, NULL, binfile_attr_set, binfile_coord_set, };
...
static long long map_binfile_download_size(struct map_priv *m)
{
	struct attr url =
	{ attr_url };
	struct attr http_method =
	{ attr_http_method };
	struct attr persistent =
	{ attr_persistent };
	struct attr *attrs[4];
	int size_ret;
	long long ret;
...
binmap_search_street_by_place(struct map_priv *map, struct item *town, struct coord *c, struct map_selection *sel)
{
	...
	{
		if (item_is_poly_place(*place) && item_attr_get(place, attr_label, &poly_town_name) && !strcmp(poly_town_name.u.str, town_name.u.str))
		{
			struct coord c[128];
			int i, count;
			found = 1;
...
binmap_search_street_by_estimate(struct map_priv *map, struct item *town, struct coord *c, struct map_selection *sel)
{
	...
	if (global_search_street_size_factor > 1)
	{
...
static int ascii_cmp(char *name, char *match, int partial)
{
	int ret = 1;
	...
	char *s2 = linguistics_casefold(match); // user entered search string
	char *s1 = linguistics_casefold(name); // string from mapfile
	char *s1_1 = linguistics_expand_special(s1, 1);
	char *s2_1 = linguistics_expand_special(s2, 1);

	if (s1_1)
	{
		if (s2_1)
		{
			ret = linguistics_compare(s1_1, s2_1, partial);
		}
		else
		{
			ret = linguistics_compare(s1_1, s2, partial);
		}
	}
	else
	{
		if (s2_1)
		{
			ret = linguistics_compare(s1, s2_1, partial);
		}
		else
		{
			ret = linguistics_compare(s1, s2, partial);
		}
	}
		...
		while ((it = map_rect_get_item_binfile(map_search->mr)))
		{
			switch (map_search->search->type)
			{
				...
				case attr_town_or_district_name:
					...
					if (map_search->mr->tile_depth > 1 && item_is_town(*it) && !item_is_district(*it) && map_search->search->type != attr_district_name)
					{
						if (binfile_attr_get(it->priv_data, attr_town_name, &at))
						{
							//// dbg(0,"22town n:%s\n",at.u.str);
							if (!ascii_cmp(at.u.str, map_search->str, map_search->partial)
							// && !duplicate(map_search, it, attr_town_name)
							)
							{
								//// dbg(0,"town n:%s\n",at.u.str);
								return it;
				...
				case attr_street_name:
					...
					if (map_search->mode == 1)
					{
						...
						if (binfile_attr_get(it->priv_data, attr_street_name, &at))
						{
							//// dbg(0,"str AA:%s\n", at.u.str);

							if (!ascii_cmp(at.u.str, map_search->str, map_search->partial))
							{
								if ((offline_search_filter_duplicates != 1) || (!duplicate(map_search, it, attr_street_name)))
								{
									return it;
								}
								else
								{
									//// dbg(0,"dup street 1:%s\n",at.u.str);
								}
							}
						}
						...
						if (binfile_attr_get(it->priv_data, attr_street_name_match, &at))
						{
							...
							if (!ascii_cmp(at.u.str, map_search->str, map_search->partial))
							{
								if ((offline_search_filter_duplicates != 1) || (!duplicate(map_search, it, attr_street_name_match)))
								{
									return it;
								}
								else
								{
									//// dbg(0,"dup street 2:%s\n",at.u.str);
								}
							}
						}
					...
					if (item_is_street(*it))
					{
						...
						if (map_selection_contains_item_rect(map_search->mr->sel, it))
						{
							if (binfile_attr_get(it->priv_data, attr_label, &at))
							{
								...
								// dbg(0,"search=%s  str is:%s\n",map_search->str, at.u.str);
								if (!ascii_cmp(str, map_search->str, map_search->partial))
								{
									match = 1;
								}
								g_free(str);
								if (match)
								{
									if ((offline_search_filter_duplicates != 1) || (!duplicate(map_search, it, attr_label)))
									{
...
{ projection_mg, "utf-8", map_destroy_binfile, map_rect_new_binfile, map_rect_destroy_binfile, map_rect_get_item_binfile, map_rect_get_item_byid_binfile, binmap_search_new, binmap_search_destroy, binmap_search_get_item, NULL, binmap_get_attr, binmap_set_attr, };
...
static int binfile_get_index(struct map_priv *m)
{
	...
	int len;
	int cde_index_size;
	long long offset; // int or long long ?
	struct zip_cd *cd;

	len = strlen("index");
	cde_index_size = sizeof(struct zip_cd) + len;
	//dbg(0,"_cde_index_size=%d\n", cde_index_size);
	if (m->eoc64)
	{
		//dbg(0,"_m->eoc64->zip64ecsz=%d\n", m->eoc64->zip64ecsz);
		offset = m->eoc64->zip64ecsz - cde_index_size;
		//dbg(0,"_eoc64 offset="LONGLONG_FMT"\n", offset);
	}
	else
	{
		//dbg(0,"_m->eoc->zipecsz=%d\n", m->eoc->zipecsz);
		offset = m->eoc->zipecsz - cde_index_size;
		//dbg(0,"_eoc offset="LONGLONG_FMT"\n", offset);
	}
	...
	if (!cd)
	{
		...
		{
			offset = m->eoc64->zip64ecsz - cde_index_size;
			//dbg(0,"_eoc64 offset aa.1="LONGLONG_FMT"\n", offset);
		}
		else
		{
			offset = m->eoc->zipecsz - cde_index_size;
			//dbg(0,"_offset 22.0="LONGLONG_FMT"\n", offset);
		}
		cd = binfile_read_cd(m, offset, len + sizeof(struct zip_cd_ext));
	}
	//dbg(0,"_offset 22="LONGLONG_FMT"\n", offset);
	...
	if (cd)
	{
		...
		if (cd->zipcfnl == len && !strncmp(cd->zipcfn, "index", len))
		{
			...
			m->index_cd = cd;
			//dbg(0,"_leave:offset 33="LONGLONG_FMT"\n", offset);
			return 1;
		}
		//dbg(0,"_cd 002\n");
	}
	...
	//dbg(0,"_leave:normal:offset 77="LONGLONG_FMT"\n", offset);
	return 1;
}
...
static int map_binfile_zip_setup(struct map_priv *m, char *filename, int mmap)
{
	...
	{
		char *tmpfilename = g_strdup(filename);
		char *ext = tmpfilename + strlen(tmpfilename) - 3;
