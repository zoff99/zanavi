...
public class IntroActivity extends AppCompatActivity
{
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		{
			pageScrollInterpolator = new LinearInterpolator();
		}
		else
		{
	...
	{
		if (isAutoplaying()) cancelAutoplay();
	}
	...
	private void smoothScrollPagerTo(final int position)
	{
		...
		animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener()
		{
			...
			private boolean fakeDragToPosition(float position)
			{
				...
				if (!pager.isFakeDragging() && !pager.beginFakeDrag()) return false;
	...
	private int nextSlideAuto()
	{
		...
		else if (pager.getCurrentItem() >= count - 1)
		{
			...
			if (autoplayCounter > 0) autoplayCounter--;
		}
		...
		if (endPosition == pager.getCurrentItem()) return 0;

		smoothScrollPagerTo(endPosition);

		if (autoplayCounter == 0) return 0;
		return distance;
	...
	private boolean canGoForward(int position, boolean notifyListeners)
	{
		...
		boolean canGoForward = (navigationPolicy == null || navigationPolicy.canGoForward(position)) && getSlide(position).canGoForward();
	...
	{
		boolean canGoBackward = (navigationPolicy == null || navigationPolicy.canGoBackward(position)) && getSlide(position).canGoBackward();
	...
	private Pair<CharSequence, ? extends View.OnClickListener> getButtonCta(int position)
	{
		if (position < getCount() && getSlide(position) instanceof ButtonCtaSlide)
		{
			...
			if (slide.getButtonCtaClickListener() != null && (slide.getButtonCtaLabel() != null || slide.getButtonCtaLabelRes() != 0))
			{
				if (slide.getButtonCtaLabel() != null)
				{
					return Pair.create(slide.getButtonCtaLabel(), slide.getButtonCtaClickListener());
				}
				else
				{
					return Pair.create((CharSequence) getString(slide.getButtonCtaLabelRes()), slide.getButtonCtaClickListener());
				}
			}
		}
		if (buttonCtaVisible)
		{
			...
			{
				return Pair.create((CharSequence) getString(buttonCtaLabelRes), new ButtonCtaClickListener());
			}
			...
			{
				return Pair.create((CharSequence) getString(R.string.mi_label_button_cta), new ButtonCtaClickListener());
			}
		}
		return null;
	}
	...
	private void updateBackground()
	{
		...
		else
		{
			...
			backgroundNext = ContextCompat.getColor(IntroActivity.this, getBackground(Math.min(position + 1, getCount() - 1)));
			...
			try
			{
				backgroundDarkNext = ContextCompat.getColor(IntroActivity.this, getBackgroundDark(Math.min(position + 1, getCount() - 1)));
			}
			catch (Resources.NotFoundException e)
			{
		...
		@ColorInt
		int backgroundButtonCta = buttonCtaTintMode == BUTTON_CTA_TINT_MODE_TEXT ? ContextCompat.getColor(this, android.R.color.white) : backgroundDarker;
		...
		if (realPosition < adapter.getCount())
		{
			...
			if (button == null)
			{
				...
				else
				{
					...
					//Fade in
					if (!((Button) buttonCta.getCurrentView()).getText().equals(buttonNext.first)) buttonCta.setText(buttonNext.first);
			...
			else
			{
				if (buttonNext == null)
				{
					buttonCta.setVisibility(View.VISIBLE);
					//Fade out
					if (!((Button) buttonCta.getCurrentView()).getText().equals(button.first)) buttonCta.setText(button.first);
	...
	private void updateButtonBackPosition()
	{
		...
		else if (realPosition < adapter.getCount() - 1)
		{
			...
			if (buttonBackFunction == BUTTON_BACK_FUNCTION_SKIP)
			{
				boolean rtl = Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1 && getResources().getConfiguration().getLayoutDirection() == View.LAYOUT_DIRECTION_RTL;
	...
	private void updateParallax()
	{
		...
		Fragment fragmentNext = position < getCount() - 1 ? getSlide(position + 1).getFragment() : null;
	...
	public void autoplay(@IntRange(from = 1) long delay, @IntRange(from = -1) int repeatCount)
	{
		...
		autoplayCallback = new Runnable()
		{
			...
			public void run()
			{
				...
				if (distance != 0) autoplayHandler.postDelayed(autoplayCallback, autoplayDelay + calculateScrollDuration(distance));
			}
		};
	...
	{
		if (listener != this.listener) pager.removeOnPageChangeListener(listener);
	}
	...
	{
		if (buttonCtaLabel != null) return buttonCtaLabel;
