...
public final class BitcoinIntegration
{
	 ...
	 * received by overriding {@link android.app.Activity#onActivityResult()}. Result indicates either {@link Activity#RESULT_OK} or {@link Activity#RESULT_CANCELED}. In the success case, use {@link #transactionHashFromResult(Intent)} to read the transaction hash from the intent.
	 * 
	 ...
	 * overriding {@link android.app.Activity#onActivityResult()}. Result indicates either {@link Activity#RESULT_OK} or {@link Activity#RESULT_CANCELED}. In the success case, use {@link #transactionHashFromResult(Intent)} to read the
	...
	/**
	 * Request payment from user, with feedback from the app. Result intent can be received by overriding {@link android.app.Activity#onActivityResult()}. Result indicates either {@link Activity#RESULT_OK} or {@link Activity#RESULT_CANCELED}. In the success case, use {@link #transactionHashFromResult(Intent)} to read the
	 * transaction hash from the intent.
	 * 
	 * Warning: A success indication is no guarantee! To be on the safe side, you must drive your own Bitcoin
	 * infrastructure and validate the transaction.
	...
	private static Intent makeBitcoinUriIntent(final String address, final Long amount)
	{
		...
		if (amount != null) uri.append("?amount=").append(String.format("%d.%08d", amount / NANOCOINS_PER_COIN, amount % NANOCOINS_PER_COIN));
	...
	private static void redirectToDownload(final Context context)
	{
		...
		try
		{
			String url_to_donate_app = "https://play.google.com/store/apps/details?id=de.schildbach.wallet";
		...
		//else if (pm.resolveActivity(binaryIntent, 0) != null) context.startActivity(binaryIntent);
		// else out of luck
	}
}
