...
navit_draw_displaylist(struct navit *this_)
{
	...
	if (this_->ready == 3)
	{
		// dbg(0,"call graphics_displaylist_draw 2")
...
navit_motion_timeout(struct navit *this_)
{
	...
	if (dx || dy) {
		...
		// dbg(0,"call graphics_displaylist_draw 1")
...
navit_zoom_out(struct navit *this_, int factor, struct point *p)
{
	//dbg(0,"EEnter\n");
	long scale=transform_get_scale(this_->trans)*factor;
	//dbg(0,"zoom out -> scale=%d",scale);
	navit_scale(this_, scale, p, 1);
}

int
navit_get_cur_pnt(struct navit *this_, struct point *p)
{
	//dbg(0,"EEnter\n");
...
void
navit_zoom_to_scale(struct navit *this_, int new_scale)
{
	//dbg(0,"EEnter\n");
...
navit_new(struct attr *parent, struct attr **attrs)
{
	...
	dbg(0,"setting center from xmlfile [hardcoded]\n");
...
void
navit_cmd_announcer_on(struct navit *this_)
{
    struct attr attr, speechattr;
...
void
navit_cmd_announcer_off(struct navit *this_)
{
    struct attr attr, speechattr;
...
void
navit_remove_all_maps(struct navit *this_)
{
	struct mapset *ms;
	struct map *map3;
...
void
navit_add_all_maps(struct navit *this_)
{
	struct mapset *ms;
	struct map *map3;
	...
	if (this_->mapsets)
	{
		...
		type.u.str="binfile";
		data.type=attr_data;
		map_file=g_strdup("/sdcard/zanavi/maps/borders.bin");
		data.u.str=map_file;

		//dbg(0,"map name=%s",map_file);
		...
		type.u.str="binfile";
		data.type=attr_data;
		map_file=g_strdup("/sdcard/zanavi/maps/coastline.bin");
		data.u.str=map_file;
		...
		for(i = 1; i < 10;i++)
		{
			...
			type.u.str="binfile";
			data.type=attr_data;
			map_file=g_strdup_printf("/sdcard/zanavi/maps/navitmap_00%d.bin",i);
			data.u.str=map_file;
			//dbg(0,"map name=%s",map_file);
			flags.type=attr_flags;
			flags.u.num=0;
		...
		for(i = 10; i < 21;i++)
		{
			...
			type.u.str="binfile";
			data.type=attr_data;
			map_file=g_strdup_printf("/sdcard/zanavi/maps/navitmap_0%d.bin",i);
			data.u.str=map_file;
			//dbg(0,"map name=%s",map_file);
			flags.type=attr_flags;
			flags.u.num=0;
...
void
navit_reload_maps(struct navit *this_)
{
...
navit_init(struct navit *this_)
{
	...
	dbg(0,"Connecting gui to graphics\n");
	...
	dbg(0,"Initializing graphics\n");
	dbg(0,"Setting Vehicle\n");
	navit_set_vehicle(this_, this_->vehicle);
	dbg(0,"Adding dynamic maps to mapset %p\n",this_->mapsets);
	if (this_->mapsets) {
		struct mapset_handle *msh;
		ms=this_->mapsets->data;
...
navit_vehicle_update(struct navit *this_, struct navit_vehicle *nv)
{
	...
	// dbg(0,"navit_vehicle_update_001 %d\n",pro);
	//profile(0,NULL);
	if (this_->ready != 3) {
		//profile(0,"return 1\n");
		return;
	...
	{
		//dbg(0,"vehicle_draw_003\n");
		navit_vehicle_draw(this_, nv, pnt);
		//global_vehicle_pos_onscreen.x=pnt->x;
		//global_vehicle_pos_onscreen.y=pnt->y;
		//dbg(0,"---> x=%d\n", pnt->x);
		//dbg(0,"---> y=%d\n", pnt->y);
	}
